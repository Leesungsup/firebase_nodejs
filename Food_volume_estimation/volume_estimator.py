...
from depth_estimation_.custom_modules import *
from depth_estimation_.project import *
...
class VolumeEstimator():
    ...
    # def classification_food(self, input_image):
    #     # cnn_model = keras.models.load_model(".\\Food_Detection\\myfood_conv3_tf1.h5")
    #     # cnn_model = keras.models.load_model(".\\Food_Detection\\0206_InceptionV3_model.h5")
    #     class_names = ['apple', 'bibimbap', 'bulgogi', 'chocolate_cake', 'fried_egg', 'hamburger', 'jajangmyeon',
    #                    'kimbap', 'kimchi_stew', 'pizza', 'ramen', 'sandwich', 'steak', 'sushi']
    #     # class_names = ['과메기','양념치킨','젓갈','콩자반','편육','피자','후라이드치킨']
    #     food_img = cv2.imread(input_image, cv2.IMREAD_COLOR)
    #     # print(input_image)
    #     # resize 방법1(224*224로 resize 한 경우)
    #     img_resize = np.resize(food_img, (1, 224, 224, 3))

    #     predictions = cnn_model.predict(img_resize)
    #     # print(predictions)
    #     pred_food = np.argmax(predictions)
    #     # print(pred_food)
    #     small_img = cv2.resize(food_img, (512, 512))
    #     # cv2.imshow("resize img", small_img)
    #     # cv2.waitKey()

    #     return class_names[pred_food]
...
if __name__ == '__main__':
    ...
    for input_image in estimator.args.input_images:
        ...
        # food = estimator.classification_food(input_image)
        food = estimator.args.food
...
#                     'chicken': [100, 293, 14.77, 48, 38, 23.86, 0], 'chocolate_cake': [100, 420, 43.6, 5.3, 25, 23],
...
#                     'steak': [200, 686, 33.79, 29.3, 46.22, 0], 'sushi': [125, 179, 37.38, 5.37, 0.52, 0]}
          food_dic = {'Galbitang': [100, 57, 14.36, 0.2, 0.03, 11.14], 'Gamjatang': [450, 537, 84.76, 30.89, 19.44, 0.22],
                    'fried chicken': [100, 293, 14.77, 48, 38, 23.86, 0], 'Gomtang_Seolleongtang': [100, 420, 43.6, 5.3, 25, 23],
                    'Gwamegi': [46, 89, 0.43, 6.24, 6, 76, 0.38], 'Soft Tofu Stew': [450, 785, 129.41, 26.81, 19.98, 6.56], 
                    'Dongtae Stew': [200, 318, 57.6, 7.3, 6.5, 0.5], 'Pyeonyuk': [125, 179, 37.38, 5.37, 0.52, 0],
                    'Kimchi Stew': [200, 121, 6.47, 7.51, 7.57, 1.77], 'pizza': [150, 404, 43.7, 15.6, 18.5, 5.4],
                    'Chicken Gyejang': [369, 515, 87.8, 11, 15.8, 0], 'Maeuntang': [146, 352, 33.35, 20.69, 15.48, 0],
                    'Samgyetang': [200, 686, 33.79, 29.3, 46.22, 0], 'Doenjang Jjigae': [125, 179, 37.38, 5.37, 0.52, 0],
                    'Seasoned Chicken':[200, 686, 33.79, 29.3, 46.22, 0],'Salted Fish': [125, 179, 37.38, 5.37, 0.52, 0],
                    'Chueotang': [125, 179, 37.38, 5.37, 0.52, 0],'Kongjaban': [125, 179, 37.38, 5.37, 0.52, 0]
                    }
        # food_dic = {'피자': [150, 404, 43.7, 15.6, 18.5, 5.4]}
        # foods = input_image.split('/')
        # print(food)
        # food = foods[-2]
        # food='피자'
        ...
        # nutrients.append(food_dic[food][2])
        # nutrients.append(food_dic[food][3])
        # nutrients.append(food_dic[food][4])
        # nutrients.append(food_dic[food][5])

        # print(food, " calorie: ", round(calorie, 5), "Kcal")
        # print("탄수화물: ", food_dic[food][2], "% ", "단백질: ", food_dic[food][3], "% ", "지방: ", food_dic[food][4], "% ",
        #       "당류: ", food_dic[food][5], "% ")q

        json_string={"dd":str(round(calorie, 5))+' , '+str(volume)+'g'}
        json_object = json.dumps(json_string)
        print(json_object)

        plt.rcParams['figure.figsize'] = [12, 8]

        # pie = plt.pie(nutrients,  ## 파이차트 출력
        #               startangle=90,  ## 시작점을 90도(degree)로 지정
        #               counterclock=False,  ## 시계 방향으로 그린다.
        #               autopct=lambda p: '{:.2f}%'.format(p),  ## 퍼센티지 출력
        #               wedgeprops=dict(width=0.5),  ## 중간의 반지름 0.5만큼 구멍을 뚫어준다.
        #               labels=["carbohydrate", "protein", "fat", "sugars"],
        #               colors=["#DF7861", "#ECB390", "#94B49F", "#FCF8E8"]
        #               )

        # plt.axis('equal')
        # plt.title("Nutrients Percents")
        # plt.show()
